    ...
    <div style={{ maxWidth: 640, margin: '0 auto', paddingTop: 20 }}>
      {step === 'activate' && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}>
          <div className="dash-card dash-card-glow" style={{ textAlign: 'center', padding: '40px 32px', background: 'linear-gradient(135deg, var(--dash-surface), rgba(99, 102, 241, 0.06))', marginBottom: 24 }}>
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2, duration: 0.5, ease: [0.34, 1.56, 0.64, 1] }} style={{ width: 80, height: 80, borderRadius: 20, margin: '0 auto 20px' }}>
              <AlfredIcon size={80} />
            </motion.div>
            <h2 style={{ fontSize: 26, fontWeight: 700, color: 'var(--dash-text)', marginBottom: 8, fontFamily: 'inherit', letterSpacing: '-0.02em' }}>Meet Alfred</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 28 }}>
            {[
              ...
              <motion.div key={label} className="dash-card" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.08, duration: 0.4, ease: [0.16, 1, 0.3, 1] }} style={{ padding: 16 }}>
          ...
          <motion.button className="dash-btn dash-btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '14px 24px', fontSize: 15 }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setStep('connect')}>
            Activate Alfred <ArrowRight size={16} />
          </motion.button>
          <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--dash-text-muted)', marginTop: 12 }}>Takes about 2 minutes. You can skip any step and do it later.</p>
        </motion.div>
      )}

      {step === 'connect' && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: 'var(--dash-text)', marginBottom: 6, fontFamily: 'inherit', letterSpacing: '-0.02em' }}>Connect Airbnb</h2>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button className="dash-btn dash-btn-primary" disabled={!airbnbEmail || !airbnbPassword || connecting} style={{ opacity: (!airbnbEmail || !airbnbPassword || connecting) ? 0.5 : 1, flex: 1, justifyContent: 'center' }}
              onClick={async () => {
                setConnecting(true)
                try {
                  // TODO: Wire to Convex connectAirbnb action
      ...
      {step === 'ready' && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }} style={{ textAlign: 'center' }}>
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2, duration: 0.5, ease: [0.34, 1.56, 0.64, 1] }}
            style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--dash-success-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px' }}>
            <Check size={32} color="var(--dash-success)" />
          </motion.div>
          <h2 style={{ fontSize: 26, fontWeight: 700, color: 'var(--dash-text)', marginBottom: 8, fontFamily: 'inherit', letterSpacing: '-0.02em' }}>Alfred is Ready</h2>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <motion.a href="https://t.me/Host4Me_bot" target="_blank" rel="noopener noreferrer" className="dash-btn dash-btn-primary" style={{ textDecoration: 'none', padding: '14px 28px', fontSize: 15 }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
              <MessageSquare size={16} /> Open Telegram <ExternalLink size={12} />
            </motion.a>
          </div>
          <button className="dash-btn dash-btn-ghost" style={{ marginTop: 16, fontSize: 13 }} onClick={() => router.push('/dashboard')}>Go to Dashboard</button>
        </motion.div>
      )}
