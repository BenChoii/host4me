    ...
    <div style={{ maxWidth: 640 }} className="dash-animate-in">
      ...
      <div className="dash-settings-section" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div
            className={`dash-toggle ${shadowMode ? 'active' : ''}`}
            role="switch"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, color: 'var(--dash-text-muted)' }}>
              Manage your profile and security via Clerk.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="dash-settings-section">
        <div className="dash-settings-title">
          <Building2 size={16} color="var(--dash-accent)" />
          Airbnb
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <AlertCircle size={14} color="var(--dash-text-muted)" />
            <span style={{ fontSize: 13, color: 'var(--dash-text-muted)' }}>
              Not connected
            </span>
          </div>
          <button className="dash-btn dash-btn-secondary" style={{ fontSize: 12 }}>
            Connect
          </button>
        </div>
      </div>
      ...
      <div className="dash-settings-section">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <AlertCircle size={14} color="var(--dash-text-muted)" />
            <span style={{ fontSize: 13, color: 'var(--dash-text-muted)' }}>
              Not connected — Alfred can't learn property details from your emails yet.
            </span>
          </div>
          <button className="dash-btn dash-btn-secondary" style={{ fontSize: 12 }}>
            Connect
      ...
      <div className="dash-settings-section">
        <div className="dash-settings-title">
          <MessageSquare size={16} color="var(--dash-accent)" />
          Telegram
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div className="dash-settings-section">
        <div className="dash-settings-title">
          <Globe size={16} color="var(--dash-accent)" />
          Communication Style
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {['Casual', 'Friendly', 'Professional', 'Luxury'].map((style) => (
            <button
              key={style}
              className={`dash-btn ${style.toLowerCase() === 'friendly' ? 'dash-btn-primary' : 'dash-btn-secondary'}`}
              style={{ fontSize: 12 }}
            >
        ...
        <div style={{ fontSize: 12, color: 'var(--dash-text-muted)', marginTop: 10 }}>
          Alfred matches this tone in all guest replies.
        </div>
      </div>
      ...
      <div className="dash-settings-section">
        <div className="dash-settings-title">
          <CreditCard size={16} color="var(--dash-accent)" />
          Billing
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 12.5, color: 'var(--dash-text-muted)', marginTop: 2 }}>
              {actionsUsed} / {actionsLimit} actions used this month
            </div>
            {/* Usage bar */}
            <div style={{
              width: 200,
              ...
              <div style={{
                width: `${(actionsUsed / actionsLimit) * 100}%`,
                height: '100%',
                borderRadius: 2,
                background: actionsUsed / actionsLimit > 0.8 ? 'var(--dash-danger)' : 'var(--dash-accent)',
                transition: 'width 0.5s ease',
              }} />
            </div>
          </div>
          <button className="dash-btn dash-btn-primary" style={{ fontSize: 12 }}>
            <Sparkles size={12} />
            Upgrade
          </button>
        </div>
