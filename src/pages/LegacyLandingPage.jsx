...
const SignInButton = ({ children }) => <div style={{cursor: 'pointer'}}>{children}</div>;
...
export default function LandingPage() {
  ...
  useGSAP(() => {
    ...
    /* ─── Color Shift ─── */
    const colorSections = [
      { trigger: '.social-proof', bg: '#080B12' },
      { trigger: '.marquee-band', bg: '#0F1420' },
      { trigger: '.mask-section', bg: '#030509' },
      { trigger: '.solution-section', bg: '#0A0F1A' },
      { trigger: '.features-section', bg: '#080B12' },
      { trigger: '.how-it-works', bg: '#0A0F1A' },
      { trigger: '.pricing-section', bg: '#0F1420' },
      { trigger: '.faq-section', bg: '#050810' },
    ];
